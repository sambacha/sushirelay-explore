...
export default function Explorer({ history }: RouteComponentProps) {
  ...
  useEffect(() => {
    if (dexSpaces !== {}) {
      socket.emit('join_to_address', {
        address: selectedCurrencyId,
        dexSpaces,
      });
      onClearStateTx();
    }
        ...
        <Row center="xs">
          <Col xs={12} sm={12} md={4} lg={4}>
            <Row center="xs" style={{ paddingTop: '0.5rem' }}>
              ...
              >
                {sortedPendingTransactions.length} {'OpenMEV Pending TXs '}
                <Emoji symbol="🔮" />
              </StyledFilterButton>

              {sortedPendingTransactions.length === 0 && (
                <LoadingViewClear txt={'Scanning Pending TXs'} />
              )}
          ...
          <Col xs={12} sm={12} md={4} lg={4}>
            ...
            <Row center="xs">
              <ButtonRefrash
                onClick={() => {
                  setDexSpaces({
                    ...dexSpaces,
                    isV2: dexSpaces.isV2 === true ? false : true,
                  });
                }}
              >
                <Text
                  fontWeight={500}
                  fontSize={16}
                  color={dexSpaces.isV2 ? theme.green1 : theme.text3}
                >
                  <UniIcon>
              ...
              <ButtonRefrash
                onClick={() => {
                  setDexSpaces({
                    ...dexSpaces,
                    isV3: dexSpaces.isV3 === true ? false : true,
                  });
                }}
              >
                <Text
                  fontWeight={500}
                  fontSize={16}
                  color={dexSpaces.isV3 ? theme.blue1 : theme.text3}
                >
                  <UniIcon>
                    <img width={'40px'} src={UniLogo} alt="logo" />
                  </UniIcon>
              ...
              <ButtonRefrash
                onClick={() => {
                  setDexSpaces({
                    ...dexSpaces,
                    isV2Sushi: dexSpaces.isV2Sushi === true ? false : true,
                  });
                }}
              >
                <Text
                  fontWeight={500}
                  fontSize={16}
                  color={dexSpaces.isV2Sushi ? theme.yellow1 : theme.text3}
                >
              ...
              <ButtonRefrash
                onClick={() => {
                  setDexSpaces({
                    ...dexSpaces,
                    is_1inch: dexSpaces.is_1inch === true ? false : true,
                  });
                }}
                style={{
                  paddingBottom: '4px',
              ...
              >
                <Text
                  fontWeight={500}
                  fontSize={16}
                  color={dexSpaces.is_1inch ? theme.primary3 : theme.text3}
                >
                  <UniIcon>
            ...
            {currency?.isToken && (
              <ToolsCurrency
                dexSpaces={dexSpaces}
                currencyAddress={selectedCurrencyId}
              ></ToolsCurrency>
            )}
            <Row center="xs">
          ...
          <Col xs={12} sm={12} md={4} lg={4}>
            <Row center="xs" style={{ paddingTop: '0.5rem' }}>
              ...
              {sortedConfirmedTransactions.length === 0 && (
                <LoadingViewClear txt={'Loading Confirmed TXs'} />
              )}
              {currency && (
                <Wrapper
                  style={{
